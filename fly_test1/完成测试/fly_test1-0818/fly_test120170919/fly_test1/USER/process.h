...
uint16_t timer5_count_500ms = 0;

/*--------------------发送打包自定义缓冲区---------------------*/
extern uint8_t uc_Pack_And_Send_Buf[11];
extern uint8_t myUart1_4g_module_data[19];

extern uint8_t AircraftModule1,AirExceptionInf,AirCustomerInf;
extern uint8_t AirLongitude[4],AirLatitude[4],AirHight[4],AirSatelliteTime[4];
...
#define RECEIVE_DATA_MAX_LENGTH  ((uint8_t)11) 	    //(2+8+2+1)
