...
{   
	...
	uint16_t Time3First=0,Time3Second=0,Time3Time = 0;
	uint8_t High_input,Low_input;
	uint16_t AirOil = 0;
//	
    // 获取定时器计数值
    strCapture .usCtr = HAL_TIM_ReadCapturedValue(&htim3,TIM_CHANNEL_1);
//		ulTime_pwmInput = strCapture .usPeriod * (0xFFFF) + strCapture .usCtr;
    if ( cntTime == 0 )
		{		
...
//			ulHighByte = Time3Time/1000;
//			ulLowByte = Time3Time%1000; 

//			uc_Pack_And_Send_Buf[4] = ulHighByte;
//			uc_Pack_And_Send_Buf[5] = ulLowByte;
//			
			AirOil  = 60/(Time3Time/1000000);
...
//			uc_Pack_And_Send_Buf[5] = (Time3Time)%100;  //低字节
//			uc_Pack_And_Send_Buf[4] = (Time3Time/100)%100;   //高字节
//			uc_Pack_And_Send_Buf[4] = Time3Time/1000;
