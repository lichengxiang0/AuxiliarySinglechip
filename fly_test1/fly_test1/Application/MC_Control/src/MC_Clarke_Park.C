...
void RevPark_Circle_Limitation(Volt_Components *Volt_Input_Output)
{
	s32 temp;
							 
	temp = (*Volt_Input_Output).qV_Component1 * (*Volt_Input_Output).qV_Component1 
             + (*Volt_Input_Output).qV_Component2 * (*Volt_Input_Output).qV_Component2;  // min value 0, max value 2*32767*32767
	...
	if ( temp > (u32)(( MAX_MODULE * MAX_MODULE) ) ) // (Vd^2+Vq^2) > MAX_MODULE^2 ?
	{
		 ...
		 temp = (s16)(*Volt_Input_Output).qV_Component1 * (u16)(index); 
		 (*Volt_Input_Output).qV_Component1 = (s16)(temp/32768);  
								
		 temp = (s16)(*Volt_Input_Output).qV_Component2 * (u16)(index); 
		 (*Volt_Input_Output).qV_Component2 = (s16)(temp/32768);  
	}
	
	/*//-----------------将/更改为右移运算changed by snow-------------------------//
	if ( temp > (u32)(( MAX_MODULE * MAX_MODULE) ) ) // (Vd^2+Vq^2) > MAX_MODULE^2 ?
	{
		 u16 index;
								
		 temp = (u32)(temp >> 24);  // min value START_INDEX, max value 127
		 temp -= START_INDEX ;   // min value 0, max value 127 - START_INDEX
		 index = circle_limit_table[(u8)temp];
								
		 temp = (s16)Stat_Volt_q_d.qV_Component1 * (u16)(index); 
		 Stat_Volt_q_d.qV_Component1 = (s16)(temp>>15);  
								
		 temp = (s16)Stat_Volt_q_d.qV_Component2 * (u16)(index); 
		 Stat_Volt_q_d.qV_Component2 = (s16)(temp>>15);  
	}*/
...
* Return         : none.
*******************************************************************************/
//  Rotor electrical angle: 0 -> 0 degrees, 
//                          S16_MAX-> 180 degrees, 
//                          S16_MIN-> -180 degrees
